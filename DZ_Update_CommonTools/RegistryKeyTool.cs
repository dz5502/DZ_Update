...
namespace DZ_Update_CommonTools
{
    ...
    public class RegistryKeyTool
    {
        ...
        private static IntPtr TransferKeyName(string keyName)
        {
        ...
        /// <returns></returns>
        private static int Set64BitRegistryKey(string key, string subKey, string name, string value)
        {
            ...
            if (ret != 0)
            {
                throw new Exception(string.Format("Unable to create key {0} - {1}: {2}!", key, subKey, ret));
            }
        ...
        #endregion
        public static void SetRegistryKey(string subKey, string name, string value)
        {
            String key = Registry.LocalMachine.Name;
            if (System.IntPtr.Size == 8)
            {
                // 写SOFTWARE\Huawei\VirtualDesktopAgent，需要关闭注册表重定向，再写64位路径的注册表
                int ret = RegistryKeyTool.Set64BitRegistryKey(key, subKey, name, value);
                if (ret != 0)
                {
                    throw new Exception(string.Format("Failed to write Reg {0}\\{1}\\{2},return {3}", key, subKey, name, ret));
                }
            }
        ...
        public static string GetRegistryValue(string path, string key)
        {
            ...
            try
            {
                ...
                if (val == null)
                {
                    //throw new Exception("Cannot find Registry key:" + key);
                    return null;
                }
